...
function buildTabularData(crateLite, layout, multiPageConfig) {
  ...
  const aboutFilterConfig = tabularConfig.addFilter && typeof tabularConfig.addFilter === "object"
    ? (Object.prototype.hasOwnProperty.call(tabularConfig.addFilter, "about")
      ? tabularConfig.addFilter.about
      : tabularConfig.addFilter["http://schema.org/about"])
    : undefined;
  const filterConfigs = [];
  const aboutFilterEnabled = aboutFilterConfig === undefined
    || aboutFilterConfig === true
    || (aboutFilterConfig && typeof aboutFilterConfig === "object" && aboutFilterConfig.addFilter !== false);

  if (aboutFilterEnabled) {
    filterConfigs.push({
      key: "about",
      label: "Filter by subject",
      propertyUri: "http://schema.org/about",
      ...(aboutFilterConfig && typeof aboutFilterConfig === "object" ? aboutFilterConfig : {}),
    });
  }
  ...
  for (const configuredType of configuredTypes) {
    ...
    const rows = entities
      .map((entity) => {
        ...
        for (const filterDef of filterConfigs) {
  ...
  const activeTypeRows = typeEntries[navType] ? typeEntries[navType].rows : [];
  const filters = filterConfigs.map((filterDef) => {
    const counts = new Map();

    for (const row of activeTypeRows) {
      const values = Array.isArray(row.filterValues[filterDef.key]) ? row.filterValues[filterDef.key] : [];
